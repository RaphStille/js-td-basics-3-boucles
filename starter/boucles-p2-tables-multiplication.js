...
const tableMulti = parseInt(prompt("Je peux vous afficher les n premières tables de multiplication. Dites-moi à laquelle vous souhaitez que je m'arrête. Par exemple, si vous entrez 4, je vous afficherai les tables de multiplication par 1, par 2, par 3 et par 4. Jusqu'à quelle table voulez-vous que j'aille ?"));
const valeur = parseInt(prompt("Combien de valeurs souhaitez-vous que j'affiche pour chacune de ces tables ?"));

if (isNaN(tableMulti) || isNaN(valeur)) {
    console.log("Vous ne pouvez rentrer que des nombres.");
}   else {
    console.log("OK, je vais vous afficher les … premières valeurs des … premières tables de multiplication.");
    for (let i =1 ; i <= valeur; i++) {
        console.log(`${tableMulti} * ${i} = ` + (tableMulti*i));
    }
    console.log(`Voice les ${valeur} premières valeurs de la table de multiplication par ${tableMulti}.`);
}